...
#define MQ136_ADC_CHANNEL ADC_CHANNEL_6     //GPIO14 (ADC2)
#define TGS2602_ADC_CHANNEL   ADC_CHANNEL_2 //GPIO2 (ADC2)
...
static int gas_read_interval = 100;
...
void RS485_SetTX()
{
    gpio_set_level(GPIO_NUM_4,1); // mengatur agar rs485 menjadi mode TX melalui pin GPIO 4 yang high
}
void RS485_SetRX()
{
    gpio_set_level(GPIO_NUM_4,0); // mengatur agar rs485 menjadi mode RX melalui pin GPIO 4 yang low
}
...
void RS485_Send(uart_port_t uart_port,uint8_t* buf,uint16_t size)
{
    ...
    uart_wait_tx_done(uart_port,portMAX_DELAY);// penambahan jeda untuk memastikan bit telah dikirim dari TX
    vTaskDelay(pdMS_TO_TICKS(5)); // memberi sedikit jeda sebelum mengembalikan TX menjadi RX
    RS485_SetRX();
}
